...
/*--------------------------------------------------------------
 * WP Store Locator – Shortcodes for contact info
 *
 * Pull address, email and hours from the first WPSL store entry
 * so the home-page Contact Us section stays in sync with the
...
function wicm_wpsl_hours_shortcode() {
    ...
    if ( $hours ) {
        ...
        // Fallback: hours stored as plain text.
        return wp_kses_post( $hours );
    }
    // Fallback to Customizer values.
